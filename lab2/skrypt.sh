...
# Tworzenie nowej resource group
az group create --location norwayeast --name wuslab

# Utworzenie sieci oraz podsieci
...
# Otwarcie portu 22 - ssh (dla Ansible)
...
# Otwarcie portu dla dostępu do frotnendu z zewnątrz
...
# Pobranie przydzielnego wyżej publiczego adresu IP
FRONTEND_HOST=$(az vm show -d -g wuslab -n wusvm --query publicIps --output tsv)

# Uzupełnienie szablonu inventory odpowiedniemi danymi: adresem IP oraz nazwą użytkownika
sed "s/HOST/$FRONTEND_HOST/;s/USER/$(whoami)/" inventory/all.template >inventory/all

# Uruchomienie Ansible dla konfiguracji nr 1
...
# Uruchomienie Ansible dla konfiguracji nr 3
if [ "$CONFIGURATION_VERSION" -eq 3 ]; then
...
# Uruchomienie Ansible dla konfiguracji nr 5
if [ "$CONFIGURATION_VERSION" -eq 5 ]; then
